...
class AdminController {
    ...
    try{
      if(db.isPgConnected()){
        const [u,c,ch,v,cv,l,b,cr,pr,au,rc,rch] = await Promise.all([
          ...
          db.query("SELECT COUNT(*) FROM users WHERE is_premium=true AND (premium_expires_at IS NULL OR premium_expires_at > CURRENT_TIMESTAMP)"), db.query("SELECT COUNT(*) FROM users WHERE account_status='active'"), db.query("SELECT COUNT(*) FROM comics WHERE publish_status='rejected'"), db.query("SELECT COUNT(*) FROM chapters WHERE publish_status='rejected'")
        ]);
        const pending = await db.query("SELECT (SELECT COUNT(*) FROM comics WHERE publish_status IN ('pending','scheduled')) AS comics, (SELECT COUNT(*) FROM chapters WHERE publish_status IN ('pending','scheduled')) AS chapters");
        ...
        res.json({totalUsers:users.length,totalComics:publishedComics.length,totalChapters:publishedChapters.length,totalViews:publishedComics.reduce((n,c)=>n+(c.views||0),0),totalChapterViews:publishedChapters.reduce((n,c)=>n+(c.views||0),0),totalLikes:db.fallbackStore.favorites.length,totalBookmarks:db.fallbackStore.bookmarks.length,totalCreators:users.filter(u=>u.role==='creator').length,totalPremiumUsers:users.filter(u=>u.is_premium && (!u.premium_expires_at || new Date(u.premium_expires_at)>new Date())).length,totalActiveUsers:users.filter(u=>(u.account_status||'active')==='active').length,rejectedSubmissions:comics.filter(c=>c.publish_status==='rejected').length+chapters.filter(c=>c.publish_status==='rejected').length,pendingComics,pendingChapters,pendingSubmissions:pendingComics+pendingChapters,systemStatus:'Dynamic Local Engine'});
      }
    }catch(err){next(err);}
  }
    ...
    try{
      if(db.isPgConnected()){
        const [c,ch]=await Promise.all([
          db.query(`SELECT c.*, u.username AS creator_username FROM comics c LEFT JOIN users u ON u.id=c.creator_id WHERE c.publish_status IN ('pending','rejected','changes_requested','scheduled') ORDER BY c.updated_at DESC`),
          db.query(`SELECT ch.*, c.title AS comic_title, c.slug AS comic_slug, u.username AS creator_username FROM chapters ch JOIN comics c ON c.id=ch.comic_id LEFT JOIN users u ON u.id=c.creator_id WHERE ch.publish_status IN ('pending','rejected','changes_requested','scheduled') ORDER BY ch.created_at DESC`)
        ]);
      ...
      const comics=db.fallbackStore.comics.filter(c=>['pending','rejected','changes_requested','scheduled'].includes(c.publish_status)).map(c=>{const u=db.fallbackStore.users.find(u=>u.id===(c.creator_id||c.creatorId));return {...Comic.formatComic(c),creatorUsername:u?.username||null};});
      const chapters=db.fallbackStore.chapters.filter(c=>['pending','rejected','changes_requested','scheduled'].includes(c.publish_status)).map(c=>{const comic=db.fallbackStore.comics.find(x=>x.id===c.comic_id);const u=db.fallbackStore.users.find(u=>u.id===(comic?.creator_id||comic?.creatorId));return {...Chapter.formatChapter(c),creatorUsername:u?.username||null};});
  ...
  static async setPremium(req,res,next){try{const user=await User.setPremium(req.params.id,!!req.body.isPremium, req.body.durationMonths || 1);if(!user)return res.status(404).json({error:'User not found.'});await FeatureService.audit({actorId:req.user.id,action:user.is_premium?'premium_granted':'premium_removed',targetType:'user',targetId:user.id,metadata:{expiresAt:user.premium_expires_at},ip:req.ip});res.json({user,message:user.is_premium?`Premium enabled for 1 month (expires ${new Date(user.premium_expires_at).toLocaleDateString()}).`:'Premium removed.'});}catch(err){next(err);}}
  static async getReports(req,res,next){try{res.json({reports:await FeatureService.reports(req.query.status||'open')});}catch(e){next(e);}}
  static async resolveReport(req,res,next){try{const row=await FeatureService.resolveReport(req.params.id,{adminId:req.user.id,status:req.body.status||'resolved',adminNote:req.body.adminNote||''});if(!row)return res.status(404).json({error:'Report not found.'});await FeatureService.audit({actorId:req.user.id,action:'report_resolved',targetType:'report',targetId:req.params.id,metadata:{status:row.status},ip:req.ip});res.json({report:row});}catch(e){next(e);}}
  static async getTickets(req,res,next){try{res.json({tickets:await FeatureService.allTickets(req.query.status||'')});}catch(e){next(e);}}
  static async updateTicket(req,res,next){try{const row=await FeatureService.updateTicket(req.params.id,req.body.status||'open',req.body.adminReply||'');if(!row)return res.status(404).json({error:'Ticket not found.'});res.json({ticket:row});}catch(e){next(e);}}
  static async createBackup(req,res,next){try{const file=await BackupService.create();res.json({message:'Backup created.',filename:require('path').basename(file)});}catch(e){next(e);}}
